    ...
    try {
        ...
        const pchResponse = await fetch(PAYCHANGU_PAYOUT_URL, {
            ...
            body: JSON.stringify({
                ...
                currency: "MWK",
                mobile: phone,
                reference: "MADA-WD-" + uid + "-" + Date.now(),
                network: network
            })
        });

        const textResponse = await pchResponse.text();
        let pchData;
        try {
            pchData = JSON.parse(textResponse);
        } catch (e) {
            return res.status(400).json({ success: false, message: "PayChangu Payout Error: " + textResponse.substring(0, 150) });
        }

        if (pchResponse.status === 200 && pchData.status === 'success') {
            return res.status(200).json({ success: true, message: "Withdrawal sent to user's phone!" });
        } else {
            let errMsg = "Unknown PayChangu Error";
            if (pchData.errors && pchData.errors.length > 0) {
                errMsg = pchData.errors[0].errorMessage + " - " + pchData.errors[0].errorDetails;
            } else {
                errMsg = pchData.errorMessage || pchData.detail || pchData.message || JSON.stringify(pchData);
            }
